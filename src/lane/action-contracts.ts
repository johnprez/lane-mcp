...
export const LaneAgentActionSchema = z.discriminatedUnion("action", [
  Base.extend({ action: z.literal("lane.create"), name: z.string().trim().min(1).max(160), description: z.string().max(8_000).default(""), status: z.enum(["planned", "active", "paused", "completed"]).default("planned"), color: Color, skipConflictCheck: z.boolean().default(false) }),
  ...
  Base.extend({ action: z.literal("milestone.create"), title: z.string().trim().min(1).max(200), description: z.string().max(8_000).default(""), status: z.enum(["planned", "in_progress", "completed", "missed", "canceled"]).default("planned"), targetDate: DateOnly.nullable().default(null), skipConflictCheck: z.boolean().default(false) }),
  ...
  Base.extend({ action: z.literal("task.create"), activityId: Id, name: z.string().trim().min(1).max(300), personIds: z.array(Id).max(200).default([]), isDone: z.boolean().default(false), note: z.string().trim().max(8_000).default(""), progress: z.number().int().min(0).max(100).default(0), dueDate: DateOnly.nullable().default(null), skipConflictCheck: z.boolean().default(false) }),
  Base.extend({ action: z.literal("task.update"), activityId: Id, taskId: Id, expectedVersion: z.number().int().positive(), name: z.string().trim().min(1).max(300), personIds: z.array(Id).max(200).default([]), isDone: z.boolean(), note: z.string().trim().max(8_000).default(""), progress: z.number().int().min(0).max(100).default(0), dueDate: DateOnly.nullable().default(null) }),
